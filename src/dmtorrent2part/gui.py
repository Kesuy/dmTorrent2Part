...
class App(tk.Tk):
    def __init__(self, initial_torrent: str | None = None) -> None:
        super().__init__()
        self.title("dmTorrent2Part - Torrent → ED2K")
    ...
    def _build_main(self, root: ttk.Frame) -> None:
        ...
        ttk.Button(root, text="打开 Torrent…", command=self._pick_torrent).grid(row=0, column=2, sticky="e")
        ttk.Label(root, textvariable=self.main_status_var, foreground="#555").grid(
            row=1, column=0, columnspan=3, sticky="w", pady=(6, 10)
        )

        local = ttk.Frame(root)
        local.grid(row=2, column=0, columnspan=3, sticky="ew", pady=(0, 8))
        ...
        ttk.Label(detail, textvariable=self.source_var, foreground="#555").grid(
        ...
        self.copy_btn = ttk.Button(detail, text="复制 ED2K", command=self._copy_link, state="disabled")
        self.copy_btn.grid(row=3, column=2, sticky="e", pady=(8, 0))
        self.hash_btn = ttk.Button(detail, text="选择本地文件计算…", command=self._pick_selected_local, state="disabled")
    ...
    def _build_legacy(self, root: ttk.Frame) -> None:
        ...
        ttk.Entry(root, textvariable=self.legacy_ed2k_var).grid(row=4, column=1, columnspan=2, sticky="ew", pady=5)
        ...
        ttk.Label(
            root,
            text="该兼容功能依赖 BitTorrent v1 SHA-1 pieces；v2-only torrent 不能用于 Part 恢复，但仍可在第一个标签页生成 ED2K。",
            wraplength=760,
            foreground="#555",
        ).grid(row=6, column=0, columnspan=3, sticky="w")
        ttk.Label(root, textvariable=self.legacy_status_var).grid(row=7, column=0, columnspan=3, sticky="w", pady=8)
    ...
    def _path_row(self, root: ttk.Frame, row: int, label: str, var: tk.StringVar, command) -> None:
    ...
    def _load_torrent(self, path: str) -> None:
        ...
        for file in torrent.files:
            if file.ed2k_hash:
                self.links[file.index] = Ed2kLink.from_hash(file.name, file.length, file.ed2k_hash)
        self.torrent_var.set(path)
        self.legacy_torrent_var.set(path)
        self._populate_tree()
        values = [f"[{f.index}] {f.path}  ({_human(f.length)})" for f in torrent.files if not f.is_pad]
        ...
        kind = "Hybrid" if torrent.is_hybrid else ("BitTorrent v2" if torrent.meta_version == 2 else "BitTorrent v1")
        text = f"{kind} · {len(torrent.files)} 个文件 · torrent 内置 ED2K：{embedded} 个"
        if torrent.warning:
            text += f" · {torrent.warning}"
        self.main_status_var.set(text)
        self.legacy_status_var.set("可进行 Part 转换" if torrent.v1_compatible else "该 torrent 仅可用于 Torrent → ED2K；不能进行 Part 转换")
    ...
    def _populate_tree(self) -> None:
        ...
        common_root = None
        if len(files) > 1:
            first_parts = [f.path.replace("\\", "/").split("/")[0] for f in files if f.path]
            if not first_parts or not all(p == torrent.name for p in first_parts):
                common_root = self.tree.insert("", "end", iid="root", text=torrent.name, open=True)
        ...
        for file in files:
            ...
            self.tree.insert(parent, "end", iid=iid, text=parts[-1] if parts else file.name, values=(_human(file.length), self._status(file)))
    ...
    def _on_tree_select(self, _event=None) -> None:
        ...
        if link:
            ...
            self.source_var.set("torrent 未包含 ED2K；请选择完整本地文件计算。" + (f" 已匹配：{local}" if local else ""))
            self._set_link_text("")
            self.copy_btn.configure(state="disabled")
    ...
    def _pick_selected_local(self) -> None:
        ...
        path = filedialog.askopenfilename(initialdir=initialdir, initialfile=target.name, filetypes=[("所有文件", "*.*")])
        if not path:
            return
        local = Path(path)
        try:
        ...
        if actual != target.length:
            messagebox.showerror("文件不匹配", f"文件大小不一致：本地 {actual} 字节，torrent {target.length} 字节。")
            return
    ...
    def _batch_hash(self) -> None:
        ...
        if not jobs:
            messagebox.showinfo("没有可计算文件", "没有找到大小匹配、且缺少 ED2K 的本地文件。")
            return
        ...
        def worker() -> None:
            try:
                for number, (target, local) in enumerate(jobs, start=1):
                    self.events.put(("hash_status", f"正在计算 {number}/{len(jobs)}：{target.path}"))
                    link = hash_file(
                        local,
                        display_name=target.name,
                        progress=lambda done, total, idx=target.index: self.events.put(("hash_progress", (idx, done, total))),
                    )
    ...
    def _sync_legacy_ed2k(self, preferred_index: int | None = None) -> None:
        ...
        if preferred_index is not None and self.torrent:
            target = next((f for f in self.torrent.files if f.index == preferred_index and not f.is_pad), None)
            if target:
                non_pad = [f for f in self.torrent.files if not f.is_pad]
                try:
    ...
    def _start_legacy(self) -> None:
        try:
            ...
            if not self.torrent.v1_compatible:
                raise ValueError("该 torrent 没有有效的 BitTorrent v1 SHA-1 pieces，不能执行 Part 转换。")
        ...
        def worker() -> None:
            try:
                result = convert(self.torrent, target, incomplete, ed2k, output, part_number=part_number, met_only=met_only)
        ...
        try:
            while True:
                ...
                if kind == "hash_status":
                    ...
                    self._busy = False
                    self.hash_btn.configure(state="normal" if self._selected_file() else "disabled")
                    ...
                    self._legacy_log_line(f"验证通过：{_human(v.verified_bytes)}；匹配分片 {v.matched_pieces}/{v.checked_pieces}")
